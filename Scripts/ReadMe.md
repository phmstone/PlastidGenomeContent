...
To run in GenBank mode, stop after `presenceAbsence.py` (`heatMapPlot.py` is optional).    
To run in discovery mode, work through all the scripts from start to end. 
GenBank mode assumes the annoatations on GenBank are correct, while discovery mode uses BLAST to pull out additional genes or gene fragements.
